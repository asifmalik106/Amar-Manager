		...
		if(func_get_arg(0)==null)
		{
			...
			$data['js'] = array('dataTables/js/jquery.dataTables.min.js',
                          'js/jquery-ui.js',
								...
								$select2Lang,
								'js/float-panel.js',
								'js/page/manager/managerSale.js',
								'js/toastr.min.js'
						);
				$this->load->model('adminModel');
				$dashModel = new adminModel();
			...
			if($reqData[0]=='get'){
				if($_POST['submit']==true && $_POST['invoiceID']!=''){
					...
					if(is_int((int)$_POST['invoiceID'])){
						...
						if($result == null){
							echo '<h1 class="text-danger text-center">Wrong Invoice Number! Please Try Again....</h1>';//404
						} 
						elseif($actualDate!=$today && $actualDate!=$yesterday) {
						    echo '<h1 class="text-danger text-center">Old Invoice can not be returned! If you want to return old invoice, please contact Administrator.</h1>';
						}
						elseif($result['invoiceType']!= "sale") {
						    echo '<h1 class="text-danger text-center">This is a Purchase Invoice. You can only return Sale Invoice!</h1>';
						} 
						else {
							$data = array(
